#define DT_DRV_COMPAT bosch_custom_bme280
...
#include <math.h>
#include <drivers/sensor/custom_bme280.h>

LOG_MODULE_REGISTER(custom_bme280, CONFIG_SENSOR_LOG_LEVEL);

/*
 * custom_bme280_data_ready_sem — signals app when sensor measurement complete
 * Weak extern: driver declares weak, app defines and owns it
 */
extern struct k_sem custom_bme280_data_ready_sem __attribute__((weak));
#define SIGNAL_DATA_READY() \
do { \
    if ((uintptr_t)&custom_bme280_data_ready_sem != 0U) { \
        k_sem_give(&custom_bme280_data_ready_sem); \
    } \
} while (0)

/* Read calibration data from sensor NVM */
static int custom_bme280_read_calib(const struct device *dev)
{
	struct custom_bme280_data *data = dev->data;
	const struct custom_bme280_config *config = dev->config;
	uint8_t calib[26];
	uint8_t calib_h[7];
...
static int32_t custom_bme280_compensate_T(struct custom_bme280_data *data, int32_t adc_T)
{
	int32_t var1, var2, T;
...
static uint32_t custom_bme280_compensate_P(struct custom_bme280_data *data, int32_t adc_P)
{
	int64_t var1, var2, p;
...
static uint32_t custom_bme280_compensate_H(struct custom_bme280_data *data, int32_t adc_H)
{
	int32_t var_H;
...
static int custom_bme280_reset(const struct device *dev)
{
	const struct custom_bme280_config *config = dev->config;
	uint8_t id;

	if (i2c_reg_read_byte_dt(&config->i2c, CUSTOM_BME280_REG_ID, &id) < 0) {
		LOG_ERR("Failed to read chip ID");
		return -EIO;
	}

	if (id != CUSTOM_BME280_CHIP_ID) {
		LOG_ERR("Unexpected chip ID: 0x%02x (expected 0x%02x)", id, CUSTOM_BME280_CHIP_ID);
		return -EINVAL;
	}

	if (i2c_reg_write_byte_dt(&config->i2c, CUSTOM_BME280_REG_RESET, CUSTOM_BME280_SOFT_RESET_CODE) < 0) {
...
static int custom_bme280_configure(const struct device *dev)
{
	const struct custom_bme280_config *config = dev->config;
	uint8_t ctrl_hum, ctrl_meas, config_reg;

	ctrl_hum = CONFIG_CUSTOM_BME280_OVERSAMPLE_HUM & 0x07;
	if (i2c_reg_write_byte_dt(&config->i2c, CUSTOM_BME280_REG_CTRL_HUM, ctrl_hum) < 0) {
		LOG_ERR("Failed to set humidity oversampling");
		return -EIO;
	}

	ctrl_meas = 0x03 |
	            ((CONFIG_CUSTOM_BME280_OVERSAMPLE_TEMP & 0x07) << 5) |
	            ((CONFIG_CUSTOM_BME280_OVERSAMPLE_PRESS & 0x07) << 2);

	if (i2c_reg_write_byte_dt(&config->i2c, CUSTOM_BME280_REG_CTRL_MEAS, ctrl_meas) < 0) {
		LOG_ERR("Failed to set measurement control");
		return -EIO;
	}

	config_reg = 0xA0;
	if (i2c_reg_write_byte_dt(&config->i2c, CUSTOM_BME280_REG_CONFIG, config_reg) < 0) {
...
static int custom_bme280_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	int retries = 10;
	do {
		ret = i2c_reg_read_byte_dt(&config->i2c, CUSTOM_BME280_REG_STATUS, &status);
		if (ret < 0) {
	...
	ret = i2c_burst_read_dt(&config->i2c, CUSTOM_BME280_REG_ADC_PRES, adc_data, sizeof(adc_data));
	if (ret < 0) {
	...
	int32_t T_fine = custom_bme280_compensate_T(data, adc_T);
	data->temperature.val1 = T_fine / 5120;
	data->temperature.val2 = (T_fine % 5120) * 195312 / 5120;

	uint32_t P = custom_bme280_compensate_P(data, adc_P);
	data->pressure.val1 = P / 256;
	data->pressure.val2 = (P % 256) * 3906;

	uint32_t H = custom_bme280_compensate_H(data, adc_H);
...
static void custom_bme280_acq_thread(const struct device *dev)
{
	LOG_INF("Custom BME280 acquisition thread started");

	while (true) {
		int ret = custom_bme280_sample_fetch(dev, SENSOR_CHAN_ALL);
		if (ret < 0) {
			LOG_ERR("Custom BME280 fetch failed: %d", ret);
		} else {
			SIGNAL_DATA_READY();
		}

		k_sleep(K_MSEC(CONFIG_CUSTOM_BME280_MEASUREMENT_INTERVAL_MS));
	}
}
#endif
...
#if CONFIG_CUSTOM_BME280_MODE_CONTINUOUS
	struct custom_bme280_data *data = dev->data;
	static uint8_t acq_stack[CONFIG_CUSTOM_BME280_ACQ_THREAD_STACK_SIZE];

	if (data->initialized) {
		LOG_WRN("Custom BME280 runtime already started");
		return -EALREADY;
	}

	k_thread_create(&data->acq_thread, acq_stack, sizeof(acq_stack),
	                (k_thread_entry_t)custom_bme280_acq_thread, (void *)dev, NULL, NULL,
	                K_PRIO_PREEMPT(CONFIG_CUSTOM_BME280_ACQ_THREAD_PRIORITY),
	                0, K_NO_WAIT);

	k_thread_name_set(&data->acq_thread, "custom_bme280");
	data->initialized = true;
	LOG_INF("Custom BME280 acquisition thread created");
	return 0;
#else
	LOG_INF("Custom BME280 ready for on-demand polling");
	return 0;
#endif
}
...
static int custom_bme280_init(const struct device *dev)
{
	...
	if (custom_bme280_reset(dev) < 0) {
		LOG_ERR("Custom BME280 reset failed");
		return -EIO;
	}

	if (custom_bme280_read_calib(dev) < 0) {
		LOG_ERR("Custom BME280 calibration read failed");
		return -EIO;
	}

	if (custom_bme280_configure(dev) < 0) {
		LOG_ERR("Custom BME280 configuration failed");
		return -EIO;
	}

	data->initialized = false;
	LOG_INF("Custom BME280 initialized (deferred start)");
	return 0;
}
...
#define CUSTOM_BME280_DEFINE(inst) \
	...
	static const struct custom_bme280_config custom_bme280_config_##inst = { \
		...
		.chip_id = CUSTOM_BME280_CHIP_ID, \
	}; \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, custom_bme280_init, NULL, \
	                              &custom_bme280_data_##inst, \
	                              &custom_bme280_config_##inst, \
	                              POST_KERNEL, \
	                              CONFIG_CUSTOM_BME280_INIT_PRIORITY, \
	                              &custom_bme280_api);
