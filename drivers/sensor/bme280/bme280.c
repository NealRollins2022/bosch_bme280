...
/*
 * SIGNAL_DATA_READY — signals app semaphore after successful acquisition.
 * bme280_data_ready_sem is declared weak extern in driver.
 * App defines and owns it. Macro is no-op if app doesn't define it.
 */
extern struct k_sem bme280_data_ready_sem __attribute__((weak));
#define SIGNAL_DATA_READY() \
do { \
    if ((uintptr_t)&bme280_data_ready_sem != 0U) { \
        k_sem_give(&bme280_data_ready_sem); \
    } \
} while (0)

/* Calibration data read from device NVM */
...
/* Compensation formulas — convert raw ADC to physical values */
...
/* Configure sensor with oversampling settings */
static int bme280_configure(const struct device *dev)
{
	...
	ctrl_hum = CONFIG_BME280_OVERSAMPLE_HUM & 0x07;
	if (i2c_reg_write_byte_dt(&config->i2c, BME280_REG_CTRL_HUM, ctrl_hum) < 0) {
		LOG_ERR("Failed to set humidity oversampling");
		return -EIO;
	}

	/* Measurement mode: normal, oversampling for T/P */
	ctrl_meas = 0x03 |  /* Mode: normal (continuous) */
	            ((CONFIG_BME280_OVERSAMPLE_TEMP & 0x07) << 5) |  /* Temp oversampling */
	            ((CONFIG_BME280_OVERSAMPLE_PRESS & 0x07) << 2);   /* Pressure oversampling */
	...
	/* Standby time: 1000ms between measurements */
	config_reg = 0xA0;  /* Standby 1000ms, no filter, SPI 3-wire disabled */
...
static int bme280_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	/* Read status — wait for measurement complete */
	uint8_t status;
	int retries = 10;
	do {
		...
		if ((status & 0x09) == 0) break;  /* measuring and updating flags clear */
		k_sleep(K_MSEC(50));
	} while (--retries > 0);
	...
	/* Read ADC data — 8 bytes from 0xF7 */
	...
	/* Apply compensation and convert to SI units */
	int32_t T_fine = bme280_compensate_T(data, adc_T);
	data->temperature.val1 = T_fine / 5120;                    /* °C */
	data->temperature.val2 = (T_fine % 5120) * 195312 / 5120;  /* °C fractional */

	uint32_t P = bme280_compensate_P(data, adc_P);
	data->pressure.val1 = P / 256;                             /* Pa */
	data->pressure.val2 = (P % 256) * 3906;                   /* Pa fractional */

	uint32_t H = bme280_compensate_H(data, adc_H);
	data->humidity.val1 = H / 1024;                            /* % */
	data->humidity.val2 = (H % 1024) * 976;                   /* % fractional */

	return 0;
}

/* Return cached sensor value — app reads via sensor_channel_get() */
...
static void bme280_acq_thread(const struct device *dev)
{
	...
	while (true) {
		...
		if (ret < 0) {
			...
			SIGNAL_DATA_READY();  /* Trigger app semaphore */
		}

		k_sleep(K_MSEC(CONFIG_BME280_MEASUREMENT_INTERVAL_MS));
	}
}
#endif
...
int bme280_runtime_start(const struct device *dev)
{
#if CONFIG_BME280_MODE_CONTINUOUS
	struct bme280_data *data = dev->data;
	static uint8_t acq_stack[CONFIG_BME280_ACQ_THREAD_STACK_SIZE];
	...
	k_thread_create(&data->acq_thread, acq_stack, sizeof(acq_stack),
	                ...
	                K_PRIO_PREEMPT(CONFIG_BME280_ACQ_THREAD_PRIORITY),
	                0, K_NO_WAIT);

	k_thread_name_set(&data->acq_thread, "bme280_acq");
...
/* Static (boot-time) initialization */
static int bme280_init(const struct device *dev)
{
	...
	data->initialized = false;  /* Deferred start */
